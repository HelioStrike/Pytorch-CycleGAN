...
class GeneratorResNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, res_blocks=9):
        ...
        for _ in range(2):
            model += [  nn.ConvTranspose2d(in_features, out_features, kernel_size=(3, 3), stride=2, padding=1, output_padding=1),
                        nn.InstanceNorm2d(out_features),
                        nn.ReLU(inplace=True)  ]
            in_features = out_features
            out_features = in_features//2
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=3):
        ...
        def discriminator_block(in_channels, out_channels, normalize=True):
            out = [ nn.Conv2d(in_channels, out_channels, 4, 2, 1) ]
            if normalize:
                out.append(nn.InstanceNorm2d(out_channels))
            out.append(nn.LeakyReLU(0.2, inplace=True))
            return out
